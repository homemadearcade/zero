...
export const addImportedArcadeGame = (arcadeGameMongoId, gameId) => async (dispatch, getState) => {
  dispatch({
  ...
  try {
    ...
    if(!gameId) gameId = getState().gameModel.gameModel.id
    const options = attachTokenToHeaders(getState);
    const response = await axios.post(`/api/arcadeGames/${gameId}/importArcadeGame`, {arcadeGameMongoId}, options);
