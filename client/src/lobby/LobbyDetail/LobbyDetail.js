    ...
    <div className="LobbyDetail">
      ...
      <div className="LobbyDetail__roles">
        ...
        <Typography component="div" variant="subtitle1">Game Host:</Typography>
          {gameSession.hostUserId && !usersById[gameSession.hostUserId] && <Typography component="div" variant="subtitle2">User is not in room</Typography>}
          {gameSession.hostUserId && usersById[gameSession.hostUserId] && <LobbyUserStatus myTracks={myTracks} userTracks={userTracks} userId={usersById[gameSession.hostUserId]?.id}/>}
          {!gameSession.hostUserId && <Typography component="div" variant="subtitle2">No Game Host Selected</Typography>}
        </div>
        <div className="LobbyDetail__role">
        ...
        <div className="LobbyDetail__others">
          ...
          <div>{lobby.users.filter((user) => {
            if(user.id ===lobby.guideId || user.id === lobby.participantId || user.id === gameSession.hostUserId) return null
            return true
