  ...
  return <div key={key} className={classnames("UserStatus", {'UserStatus--left' : !user.joined, 'UserStatus--cobrowser': cobrowsingUser.id === userId})}>
    ...
    <AccordianList accordians={[{
      ...
      body: <div className="UserStatus__icons">
        <div className="UserStatus__fullscreen"><div className="UserStatus__icon"><Icon icon="faWindowMaximize"/></div>{(userStatus?.isFullscreen) ? 'Fullscreen' : 'Windowed'}</div>
        <div className="UserStatus__focus"><div className="UserStatus__icon"><Icon icon="faEye"/></div>{(!userStatus || userStatus?.isFocused) ? 'On Tab' : 'Away'}</div>
        <div className="UserStatus__cobrowsing"><div className="UserStatus__icon"><Icon icon="faArrowPointer"/></div>{userCobrowsingStatus ? <span>{((Date.now() - userCobrowsingStatus.lastPing)/1000).toFixed(0)}s ago</span> : 'Never'}</div>
        <div className="UserStatus__upload"><div className="UserStatus__icon"><Icon icon="faUpload"/></div>{(user.internetSpeedTestResults?.uploadSpeed) ? user.internetSpeedTestResults?.uploadSpeed : 'Not Tested'}</div>
        <div className="UserStatus__download"><div className="UserStatus__icon"><Icon icon="faDownload"/></div>{(user.internetSpeedTestResults?.downloadSpeed) ? user.internetSpeedTestResults?.downloadSpeed : 'Not Tested'}</div>
        <div className="UserStatus__video-call"><div className="UserStatus__icon"><Icon icon="faVideo"/></div>{userTracksById && userTracksById[user.id] ? 'Connected' : 'Not Connected'}</div></div>
    }]}/>
  </div>
};
