...
import { defaultClass, defaultClassByCategory, defaultEntityInstance, defaultStage, defaultTag, effectBehaviorInterfaces, EFFECT_CUTSCENE, EFFECT_DESTROY, EFFECT_SPAWN, EFFECT_TELEPORT, EFFECT_TRANSFORM, initialTags, libraryClassAugment, ON_INTERACT, playerRelationTagId, RELATION_TAG_CLASS, RELATION_TAG_CUTSCENE, RELATION_TAG_DIALOGUE, RELATION_TAG_LIBRARY, RELATION_TAG_TELEPORT, RELATION_TAG_TRANSFORM } from "../game/constants"
import { BRUSH_ID_PREFIX, POWERUP_CLASS } from "../game/constants/core"
import { ZONE_CLASS } from "../game/constants/core"
import { NON_LAYER_BRUSH_ID } from "../game/constants/core"
import { initialStageZoneClassId } from "../game/constants/core"
import { PLAYER_CLASS } from "../game/constants/core"
import { mergeDeep } from "../utils/utils"

export function addAutogeneratedClassesToGameModel(gameData) {
  if(gameData.entityClasses) {
    Object.keys(gameData.entityClasses).forEach((entityClassId) => {
      ...
      if(entityClass.classInterfaceCategory === PLAYER_CLASS) {
        const powerUpClassId = 'transform-'+entityClassId
        gameData.entityClasses[powerUpClassId] = {
          isAutogenerated: true,
          name: 'Transform Player into ' + entityClass.name,
          relationTags: {
            [powerUpClassId]: {
              wasAutoapplied: true
            }
          ...
          classInterfaceCategory: POWERUP_CLASS,
          entityClassId: powerUpClassId,
          graphics: {
...
export function enrichGameModel(gameData) {
  ...
  Object.keys(gameData.entityClasses).forEach((entityClassId) => {
    ...
    const transformIntoEffect = entityClass.autogeneration.transformIntoEffect
    if(transformIntoEffect) {
      ...
      const playerTransformIntoRelationTag = entityClass.autogeneration.playerTransformIntoRelationTag
      if(playerTransformIntoRelationTag && entityClass.classInterfaceCategory === PLAYER_CLASS) {
    ...
    const teleportToEffect = entityClass.autogeneration.teleportToEffect
    if(entityClass.classInterfaceCategory === ZONE_CLASS && teleportToEffect) {
      ...
      gameData.effects[teleportClassId] = {
        ...
        zoneEntityClassId: entityClassId,
        isAutogenerated: true
      }

      const playerTeleportToRelationTag = entityClass.autogeneration.playerTeleportToRelationTag
    ...
    const spawnAnywhereEffect = entityClass.autogeneration.teleportToEffect
    if(spawnAnywhereEffect) {
      const spawnEntityClassId = 'spawn-'+entityClassId
      gameData.effects[spawnEntityClassId] = {
        effectId: spawnEntityClassId,
        effectBehavior: EFFECT_SPAWN,
        zoneEntityClassId: initialStageZoneClassId,
        spawnEntityClassId: entityClassId,
        isAutogenerated: true,
      }
    }
    ...
    if(entityClass.autogeneration.automaticClassTag) {
      gameData.relationTags[entityClass.entityClassId] = {
        relationTagInterfaceType: RELATION_TAG_CLASS,
        isAutogenerated: true,
        textureId: entityClass.graphics.textureId,
        isSelectorObscured: entityClass.autogeneration.isSelectorObscured,
        textureTint: entityClass.graphics.textureTint,
  ...
  Object.keys(gameData.relationTags).forEach((relationTagId) => {
    ...
    if(relationTag.relationTagInterfaceType === RELATION_TAG_CLASS || relationTag.relationTagInterfaceType === RELATION_TAG_LIBRARY) {
      if(relationTagClass?.destroyAllEffect) {
