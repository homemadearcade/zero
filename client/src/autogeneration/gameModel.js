...
import { effectInterfaceData, EFFECT_CUTSCENE, EFFECT_DESTROY, EFFECT_SPAWN, EFFECT_TELEPORT, EFFECT_TRANSFORM,   
  ON_INTERACT, PLAYER_RELATION_TAG_ID,
   eventInterfaceData, STAGE_ZONE_ENTITY_RID, PLAYTHROUGH_START_CUTSCENE_RID, END_GAME_CUTSCENE_RID, 
 } from "../game/constants"
...
export function generateActionEffects(gameModel) {
  ...
  Object.keys(interfaceIdData).forEach((interfaceId) => {
    ...
    const interfaceGroupIdData = interfaceGroupData[interfaceData.interfaceGroupId]
    if(!name) return
    ...
    gameModel.effects[effectId] = {
      ...
      group: 'Unlock ' + interfaceGroupIdData.name + ' UI',
      title: 'Unlock ' + name,
      isReadOnly: true,
   ...
   Object.keys(interfaceActionData).forEach((interfaceActionId) => {
    const actionIdData = interfaceActionData[interfaceActionId]

    if(!actionIdData.arguments) {
      const effectId = EFFECT_DID + interfaceActionId
      const actionGroupIdData = interfaceActionGroupData[actionIdData.interfaceActionGroupId]
      if(!actionGroupIdData) return console.error('no interface action ui data for ' + interfaceActionId)
      gameModel.effects[effectId] = {
        effectId,
        effectBehavior: EFFECT_INTERFACE_ACTION,
        interfaceActionId,
        dataSourceIID: DERIVED_INTERFACE_ACTION_IID,
        title: actionIdData.title,
        subTitle: actionIdData.subTitle,
        onClick: actionIdData.onClick(),
        isActive: actionIdData.isActive,
        icon: actionIdData.icon || actionGroupIdData.icon,
        isReadOnly: true
      }
    } else {
      actionIdData.arguments.forEach((arg1, arg2) => {
        ...
        if(arg1 === 'entityModelId') {
          ...
          if(gameModel.entityModels) Object.keys(gameModel.entityModels).forEach((entityModelId) => {
            ...
            if(actionIdData.isRemoved) {
              isActionRemoved = actionIdData.isRemoved([entityModelId], gameModel)
            }
            ...
            gameModel.effects[effectId] = {
              ...
              interfaceActionId,
              dataSourceIID: DERIVED_INTERFACE_ACTION_IID,
              subTitle: actionIdData.getSubtitle([entityModelId], gameModel),
              title: actionIdData.title || actionIdData.getTitle([entityModelId], gameModel),
              onClick: actionIdData.onClick([entityModelId]),
              entityModelId,
              isReadOnly: true,
          ...
          if(gameModel.layers) Object.keys(gameModel.layers).forEach((layerId) => {
            ...
            gameModel.effects[effectId] = {
              ...
              dataSourceIID: DERIVED_INTERFACE_ACTION_IID,
              layerId,
              subTitle: actionIdData.getSubtitle([layerId], gameModel),
              title: actionIdData.title || actionIdData.getTitle([layerId], gameModel),
              onClick: actionIdData.onClick([layerId]),
              isReadOnly: true
            }
          })
          ...
          if(gameModel.brushes) Object.keys(gameModel.brushes).forEach((brushId) => {
            ...
            gameModel.effects[effectId] = {
              ...
              dataSourceIID: DERIVED_INTERFACE_ACTION_IID,
              subTitle: actionIdData.getSubtitle([brushId], gameModel),
              title:  actionIdData.title || actionIdData.getTitle([brushId], gameModel),
              onClick: actionIdData.onClick([brushId]),
              isReadOnly: true
            }
          })
          ...
          if(gameModel.colors) Object.keys(gameModel.colors).forEach((colorId) => {
            ...
            Object.keys(color).forEach((layerId) => {
              ...
              gameModel.effects[effectId] = {
                ...
                textureTint: colorId,
                dataSourceIID: DERIVED_INTERFACE_ACTION_IID,
                subTitle: actionIdData.getSubtitle([colorId, layerId], gameModel) ,
                title: actionIdData.title || actionIdData.getTitle([colorId, layerId], gameModel),
                onClick: actionIdData.onClick([colorId, layerId]),
                isReadOnly: true
              }
            })
          ...
          if(gameModel.stages) {
            Object.keys(gameModel.stages).forEach((stageId) => {
              ...
              gameModel.effects[effectId] = {
                ...
                stageId,
                dataSourceIID: DERIVED_INTERFACE_ACTION_IID,
                subTitle: actionIdData.getSubtitle([stageId], gameModel),
                title: actionIdData.title || actionIdData.getTitle([stageId], gameModel),
                onClick: actionIdData.onClick([stageId]),
                isReadOnly: true
              }
            })
          }
...
export function enrichGameModel(gameData) {
  ...
  Object.keys(effectInterfaceData).forEach((effectBehavior) => {
    const effectTypeInterfaceData = effectInterfaceData[effectBehavior]
    const effectName = effectTypeInterfaceData.displayName

    if(effectTypeInterfaceData.autogenerateEffect) {
      gameData.effects[effectBehavior] = {
        ...
        isCommonlyUsed: true,
        isReadOnly: true,
        group: effectTypeInterfaceData.group,
      }

      if(effectTypeInterfaceData.autogenerateRelationForEvents) {
        effectTypeInterfaceData.autogenerateRelationForEvents.forEach((eventType) => {
          ...
          const targetableType = effectInterfaceData[effectBehavior].targetableType
          ...
          targetName.forEach((targetName) => {
            ...
            if(eventTypeInterface.relationTagSelectType === SINGLE_RELATION_TAG_EVENT_IID) {
              gameData.relationTags[eventId] = {
                ...
                isReadOnly: true,
                icon: effectTypeInterfaceData.icon
                // editorInterface: {
    ...
    if(teleportToEffect) {
      ...
      if(playerTeleportToRelationTag) {
        ...
        gameData.relationTags[teleportEntityId] = {
          ...
          icon: effectInterfaceData[EFFECT_TELEPORT].icon,
