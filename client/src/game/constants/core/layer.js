export const editorHighlightDepthModifier = -1
export const interactAreaDepthModifier = 1
export const invisibleIndicatorDepthModifer = 2
export const playerDepthModifier = 3 

export const NON_LAYER_COLOR_ID = 'NON_LAYER_COLOR_ID';
export const NON_LAYER_BRUSH_ID = 'NON_LAYER_BRUSH_ID'
export const NON_LAYER_BRUSH_DEPTH = 71

export const STAGE_LAYER_DEPTH = 0;

export const BACKGROUND_LAYER_GROUP_DEPTH = 100;
export const PLAYGROUND_LAYER_GROUP_DEPTH = 300;
export const FOREGROUND_LAYER_GROUP_DEPTH = 500;

export const CANVAS_IMAGE_DIALOG_LAYER_DEPTH = 700;
export const UI_LAYER_DEPTH = 900;

export const STAGE_LAYER_ID = 'STAGE_LAYER_ID';
export const BACKGROUND_LAYER_ID = 'BACKGROUND_LAYER';
export const PLAYGROUND_LAYER_ID = 'PLAYGROUND_LAYER';
export const FOREGROUND_LAYER_ID = 'FOREGROUND_LAYER';
export const CANVAS_IMAGE_LAYER_ID = 'CANVAS_IMAGE_LAYER_ID';
export const UI_LAYER_ID = 'UI_CANVAS';