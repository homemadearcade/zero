  ...
  function generateTextureIdsByDescriptors() {
    ...
    Object.keys(window.spriteSheets).forEach((spriteSheetId) => {
      ...
      ss.sprites.forEach((s, i) => {
        if(!s.visualTags) return
  
        let spriteMatchesComplex = false
        Object.keys(global.complexDescriptors).forEach((desc) => {
          const visualTag = global.complexDescriptors[desc]
          let failed = false
          Object.keys(visualTag.withDescriptors).forEach((withD) => {
            if(!s.visualTags[withD]) {
              failed = true
            }
          })
        ...
        // if it matches a complex visualTag, you can get it from there, easy
        ...
        if(s.visualTags) {
          ...
          let spriteAdditions = []
          Object.keys(s.visualTags).forEach((desc) => {
            if(!s.visualTags[desc]) return
  
            // mods should normally be used to create a complex version of a sprite, if a mod visualTag has reached this point it is missing a complex visualTag
            // later we generate a visualTag for it
            ...
            // directional modfiers dont get a generated visualTag
    ...
    Object.keys(global.missingComplexDescriptors).forEach((modifier) => {
      ...
      sprites.forEach((sprite) => {
        let nonModifier
        Object.keys(sprite.visualTags).forEach((desc) => {
          if(!sprite.visualTags[desc]) return
        ...
        if(!global.spriteIdsByDescriptor[name]) {
          global.allDescriptors[name] = {
            withDescriptors: sprite.visualTags,
...
export function getDescriptors() {
  ...
  // this ones will auto group if there is no matching complexity visualTag
