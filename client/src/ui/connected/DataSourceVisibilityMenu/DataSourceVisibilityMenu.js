...
import { DATA_SOURCE_AUTOGENERATED_IID, DATA_SOURCE_CORE_LIBRARY_IID, DATA_SOURCE_ENTITY_MODEL_IID, IS_DATA_REMOVED_IID,RELATION_TAG_ABSTRACT_IID, SELECT_RELATION_TAG_IID } from "../../../constants/interfaceIds";
...
function DataSourceVisibilityMenu({ interfaceId, gameSelector: { selectorInterfaceListInvisibility }, toggleSelectorClassInvisibility, cobrowsing: { isActivelyCobrowsing, isSubscribedCobrowsing } }) {
  ...
  function renderSelectorListCheckbox(dataSource) {
    return <Checkbox size="small" icon={dataSourceToIcon[dataSource]} checked={!selectorInterfaceListInvisibility[interfaceId]?.[dataSource]} onChange={() => {
      toggleSelectorClassInvisibility(interfaceId, dataSource)
  ...
  function getSelectorListVisibility() {
    ...
    // if(interfaceId !== DIALOGUE_ABSTRACT_IID && interfaceId !== CUTSCENE_ABSTRACT_IID) {
      ...
      menuItems.push(
        <MenuItem key={DATA_SOURCE_CORE_LIBRARY_IID+interfaceId} dense>{renderSelectorListCheckbox(DATA_SOURCE_CORE_LIBRARY_IID)}</MenuItem>
      )
      ...
      // menuItems.push(
      //   <MenuItem>{renderSelectorListCheckbox(DATA_SOURCE_USER_LIBRARY_IID)}</MenuItem>
      // )
    // }
