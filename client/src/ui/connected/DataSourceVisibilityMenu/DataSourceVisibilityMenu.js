...
import { DATA_SOURCE_AUTOGENERATED_IID, DATA_SOURCE_IMPORTED_GAME_MODE_IID, DATA_SOURCE_ENTITY_MODEL_IID, IS_DATA_REMOVED_IID,RELATION_TAG_ABSTRACT_IID, SELECT_RELATION_TAG_IID, DATA_SOURCE_SYSTEM_IID, DATA_SOURCE_ACTION_IID, EFFECT_ABSTRACT_IID } from "../../../constants/interfaceIds";
...
function DataSourceVisibilityMenu({ interfaceId, gameSelector: { selectorInterfaceListInvisibility }, toggleSelectorClassInvisibility, cobrowsing: { isActivelyCobrowsing, isSubscribedCobrowsing } }) {
  ...
  function getSelectorListVisibility() {
    ...
    // if(interfaceId !== DIALOGUE_ABSTRACT_IID && interfaceId !== CUTSCENE_ABSTRACT_IID) {
      ...
      if(interfaceId === EFFECT_ABSTRACT_IID) {
        menuItems.push(
          <MenuItem key={DATA_SOURCE_ACTION_IID+interfaceId} dense>{renderSelectorListCheckbox(DATA_SOURCE_ACTION_IID)}</MenuItem>
        )
      }
